...
import { generarTextoConEspeciales } from '../utils/valorAleatorio';
...
test.describe('API - Crear guía de Recaudo', () => {
  ...
  test.beforeAll(async () => {
    ...
    consultarGuia = new ConsultarGuiaPage(requestContext);
      ...
      test('Crear guía con referencia con caracteres especiales y validar vía GET', async () => {
  
      const guiaModificada = { ...guiaBase };
      guiaModificada.referenciaRecaudo = generarTextoConEspeciales().toString(); 
